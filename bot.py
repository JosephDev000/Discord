...
@client.command() # .dasnike command
async def daddy():
	await client.say("Navarro is daddy and mommmy xxxxxxxxx =D")	

@client.command(pass_context=True) #Bot Version with Owner permission
async def info(ctx):
	if ctx.message.author.id ==(creatorid):
		await client.say(bot_version_credits)
	else:
		await client.say("No permission. This command can only be used by the bot creator.")
